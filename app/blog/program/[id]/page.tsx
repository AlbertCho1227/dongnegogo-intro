        ...
        <article>
          ...
          <div className="blog-article__body">
            ...
            {images.length > 1 && <section className="blog-media-story" aria-labelledby="program-media-title">
              ...
              <div className="blog-media-grid">{images.slice(1).map((image, index) => <figure key={image.url}><span className="blog-program-media-fallback" aria-hidden="true"><img src={icon} alt="" /></span><ProgramMediaImage src={image.thumbnailUrl || image.url} fallbackSrc={icon} alt={`${program.name} ${image.role.includes("facility") ? "시설" : "프로그램"} 사진 ${index + 1}`} loading="lazy" /><figcaption><strong>{image.role.includes("facility") ? "시설 사진" : "프로그램 이미지"}</strong><span>출처: {visibleSource(image, program.source)}</span>{image.licenseUrl ? <a href={image.licenseUrl} target="_blank" rel="noreferrer">이용조건 확인 ↗</a> : image.license && <span>{image.license}</span>}</figcaption></figure>)}</div>
            </section>}
