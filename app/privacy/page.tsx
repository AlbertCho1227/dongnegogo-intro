...
export default function PrivacyPage() {
  return (
    <LegalPage eyebrow="개인정보 보호" title="개인정보처리방침" description="동네고고는 필요한 정보만 처리하고, 계정과 위치 정보의 흐름을 숨김없이 설명합니다.">
      <LegalMeta>공개일 2026년 8월 11일 · 최종 수정·시행 2026년 8월 30일 · 「개인정보 보호법」 제30조에 따른 개인정보처리방침</LegalMeta>
      ...
      <LegalSection title="6. 파기와 보호조치">
        ...
        <p>인증·사용자 데이터는 행 수준 보안(RLS), 로그인 사용자 UUID 소유권 확인, 최소 SQL 권한, 전송구간 암호화, 비밀키 분리와 접근 통제로 보호합니다. 비밀번호 로그인을 직접 구현하거나 비밀번호 원문을 저장하지 않으며, 인증 비밀번호가 필요한 경우 Supabase Auth의 단방향 해시 저장을 사용합니다.</p>
        <p>로그인 세션은 iOS에서 Keychain, Android에서 Android Keystore 기반 AES-GCM 암호화 저장소를 사용합니다. 웹·앱인토스처럼 브라우저 저장소를 사용하는 환경은 비밀 관리자 키를 포함하지 않고, 출처가 다른 스크립트·연결을 콘텐츠 보안 정책(CSP)으로 제한하며 로그아웃·계정 삭제 시 계정 저장값을 정리합니다.</p>
