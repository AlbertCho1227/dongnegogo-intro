...
function archiveHref(page: number, category: string, query: string): string {
...
export function BlogPagination({ page, totalPages, category, searchTerm }: {
  page: number;
  totalPages: number;
  category: string;
  searchTerm: string;
  ...
  const previousHref = page > 1 ? archiveHref(page - 1, category, searchTerm) : "";
  const nextHref = page < totalPages ? archiveHref(page + 1, category, searchTerm) : "";
  ...
  return <nav className="blog-pagination" aria-label="프로그램 글 페이지" aria-busy={Boolean(pendingHref)}>
    ...
    <div>{pages.map((item, index) => {
      const href = archiveHref(item, category, searchTerm);
