...
test("오픈런 화면은 현재 도시 목록을 대기 화면 없이 즉시 나눠 표시한다", async () => {
  const source = await readFile(new URL("app/web/web-map-app.tsx", projectRoot), "utf8");
  ...
  assert.doesNotMatch(source, /setContentReady/);
  assert.doesNotMatch(source, /오픈런 프로그램을 준비하고 있어요/);
  assert.match(source, /cityName=\{openRunCityLabel\(openRunCity\)\}/);
  assert.match(source, /className="dg-openrun-scroll-top"/);
